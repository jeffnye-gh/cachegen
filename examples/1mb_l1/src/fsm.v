...
(
  ...
  input  wire        pe_read,      //input from CPU
  input  wire        pe_write,     //input from CPU
  ...
  output wire [TAG_BITS-1:0] fsm_cc_tag,
  output wire [IDX_BITS-1:0] fsm_cc_index,
  output wire [2:0]          fsm_cc_offset,
  output wire [3:0]          fsm_cc_be,
  ...
  output wire [3:0]          fsm_cc_way_match,
...
//synthesis on
// ----------------------------------------------------------------------
assign fsm_cc_tag    = pe_a[31:18];
assign fsm_cc_index  = pe_a[17:5];
assign fsm_cc_offset = pe_a[4:2];

reg fsm_pe_readdata_valid_d;
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
reg [3:0] state,next;
// ----------------------------------------------------------------------
...
wire pe_req_hit = pe_access & |way_hit;
...
always @(posedge clk) begin
  ...
  pe_access <= pe_access_d;
end
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
always @* begin
  fsm_pe_readdata_valid_d = 1'b0;
  ...
  casez(state) 
    IDLE: begin
      // READ HIT
      if(pe_read ) begin //&  pe_req_hit)  begin
      ...
      // READ MISS CLEAN
      else if(pe_read & !pe_req_hit & !pe_req_mod) begin
        next = RD_ALLOC;
      end

      // WRITE MISS CLEAN
      else if(pe_write & !pe_req_hit &  pe_req_mod) begin
        next = WR_ALLOC;
      end
