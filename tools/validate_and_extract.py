...
def check_ia_session(header_lines):
    """
    Check that IA session field is present in the header
    table and is not empty or the placeholder value 'NNN'.
    Returns list of warning strings (empty if all good).
    Warns but does not fail -- IA session may be absent in
    pre-backfill files.
    """
    errors = []
    found = False
    for line in header_lines:
        ...
        if not stripped.startswith('|'):
            continue
        cols = [c.strip() for c in stripped.split('|')]
        if len(cols) >= 3 and cols[1].lower() == "ia session":
            found = True
            val = cols[2]
            if not val or val.upper() == 'NNN':
                errors.append(
                    "  [IA session] IA session field is "
                    "empty or placeholder -- fill in the "
                    "IA session number before submitting."
                )
            break
    if not found:
        errors.append(
            "  [IA session] IA session field absent from "
            "header table -- add '| IA session | NNN |' "
...
def validate(lines):
    ...
    # -- 3. IA session (warn, not fail) --
    errors += check_ia_session(header_lines)
